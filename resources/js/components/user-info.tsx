...
export function UserInfo({ user, showEmail = false }: { user?: User | null; showEmail?: boolean }) {
    ...
    return (
        <>
            <Avatar className="h-8 w-8 overflow-hidden rounded-full border border-white/15 shadow-sm">
            ...
            <div className="grid flex-1 text-left text-sm leading-tight">
                <span className="truncate font-semibold text-slate-200">{user?.name || 'Guest'}</span>
                {showEmail && <span className="text-slate-400 truncate text-xs">{user?.email || ''}</span>}
            </div>
        </>
    );
}
